...
@register.simple_tag
def paginator_number(page_obj, i, paginate_by, f, q, o):
    """
    ...
    if i == DOT:
        ...
        return format_html('<li class="page-item active"><a class="page-link" href="#">{0}<span class="sr-only">(current)</span></a></li> ', i)
    else:
        return format_html('<li class="page-item"><a class="page-link" href="{0}"{1}>{2}</a> </li>',
                           '?page=%s&paginate_by=%s&f=%s&q=%s&o=%s' % (i, paginate_by, f, q, o),
                           mark_safe(' class="end"' if i == page_obj.paginator.num_pages else ''), i)
...
def paginator(context, adjacent_pages=2):
    ...
    paginator, page_num = context['page_obj'].paginator, context['page_obj'].number
    page_range = []
