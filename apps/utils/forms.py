...
"""
@copyright   Copyright (c) Aldo AC
@author      Aldo AC (@gitdealdo)
@package     utils
...
TEMPLATE_PACK = getattr(settings, 'CRISPY_TEMPLATE_PACK', 'bootstrap4')
...
def btnCancel():
    return Button(
        ...
        ugettext('Cancelar'),
        css_class='btn btn-danger btn-back text-bold', title='%s' % ugettext('Cancel'), onclick='history.back()')
...
def btnReset():
    return Reset(
        ...
        ugettext('Limpiar'),
        css_class='btn btn-outline-secondary text-bold', title='%s' % ugettext('Reset'))
